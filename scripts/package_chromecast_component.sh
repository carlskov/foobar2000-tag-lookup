...
if [[ ! -f "$INPUT_BIN" ]]; then
  ...
  echo "Build first: cmake -S . -B build && cmake --build build --target foo_chromecast"
  exit 1
fi
...
<dict>
  ...
  <string>foo_chromecast</string>
  <key>CFBundleIdentifier</key>
  <string>com.foobar2000.foo-chromecast</string>
